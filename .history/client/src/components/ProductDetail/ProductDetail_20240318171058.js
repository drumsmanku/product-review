...
function ProductDetail() {
  ...
  useEffect(() => {
    localStorage.setItem('isTeamMember', isTeamMember);
  }, [isTeamMember]);
  
  const admin=localStorage.getItem('isAdmin');
  const teamMember=localStorage.getItem('isTeamMember');
    ...
    <div style={{width:'100vw', height:'100vh', display:'flex', justifyContent:'center', alignItems:'center', border:'2px solid black'}}>
     ...
     <Box
      ...
      >
        ...
        {admin==='true'&&<button type='button' onClick={handleProductSubmit} style={buttonStyles}>Update Product as Admin</button>}
        {teamMember==='true'&&
