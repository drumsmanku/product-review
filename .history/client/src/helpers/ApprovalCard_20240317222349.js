...
function ApprovalCard({data}) {
  ...
  const handleAccept=()=>{
    ...
    } ).then(()=>{setIsUpdated(prev => !prev); navigate('/dashboard-admin')}).catch(err=>console.log(err));
  ...
  const handleReject=()=>{
    axios.patch(`http://localhost:4000/products/review/${data.id}/status`,{status:"rejected"})
          .then(() => {
            setIsUpdated(prev => !prev);
            navigate('/dashboard-admin');
          })
