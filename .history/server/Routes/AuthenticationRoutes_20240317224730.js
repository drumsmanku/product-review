...
router.post('/signup', async (req, res) => {
  const { name, email, mobile, password, isAdmin } = req.body; // Added isAdmin field
  try {
...
router.post('/login',async (req, res)=>{
  ...
  try{
    ...
    if(existingUser){
      ...
      res.send({message:"user exists, Signed in successfully",jwtToken, name: userInDB.name, isAdmin:userInDB.isAdmin })
    }
    else{
